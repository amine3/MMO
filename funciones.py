...
def cortar_icon(icon_name, len_image_source, width, height, debut_x, debut_y):
    image = load_image(icon_name, alpha=True)
    rect = image.get_rect()
    sprite = []
    w = width
    h = height
    start_x = debut_x
    start_y = debut_y
    for i in range(len_image_source):
        sprite.append(image.subsurface((start_x, start_y, w, h)))
        start_x += w
    return sprite


def cut_image(image, start_x, start_y, width, heigh):
    image_orig = load_image(image, alpha=True)
    image_cut = image_orig.subsurface(start_x, start_y, width, heigh)
    return image_cut
