...
class InterfazJuego:
    def __init__(self, screen):
        ...
        self.jugador = Personaje("Amine", velocidad, WIDTH, HEIGHT, "Graphics/charasets/char.png",
                                 "Graphics/battle\party/felix/Felix_Axe_Back.gif",
                                 "Graphics/battle\party/felix/Felix_Axe_Back.gif")
        self.screen = screen
        self.FastSurfaces = []

        #Barra Estado Info        
        HP = Text(text=(str(self.jugador.hp) + "/" + str(self.jugador.maxhp)), size=13, color=(255, 255, 255), left=190,
                  top=5)
        SP = Text(text=(str(self.jugador.sp) + "/" + str(self.jugador.maxsp)), size=13, color=(255, 255, 255), left=190,
                  top=35)
        EXP = Text(text=(str(self.jugador.exp) + "/" + str(self.jugador.exp_nivel)), size=13, color=(255, 255, 255),
                   left=190, top=65)
        ...
        self.BagWindow = Window(width=bag_image.rect.w, height=bag_image.rect.h, left=WIDTH - 170, top=HEIGHT - 300,
                                moveable=True)
        ...
        for y in range(6):
            for x in range(5):
                ...
                self.BagWindow.appendReceptor(
                    Receptor(Type="Inventory", left=10 + x * 27, top=32 + y * 27, width=24, height=24))
        ...
        char_w_exp = Text(text="Exp: " + str(round(self.jugador.exp * 100.0 / self.jugador.exp_nivel, 2)) + "%",
                          size=12, color=(209, 195, 137), left=100, top=53)
        char_w_prof = Text(text="Job: " + self.jugador.especializacion, size=12, color=(209, 195, 137), left=20,
                           top=53 + 15)
        char_w_str = Text(text="Str: " + str(self.jugador.str + self.jugador.str_b), size=12, color=(209, 195, 137),
                          left=20, top=74 + 15)
        char_w_dex = Text(text="Dex: " + str(self.jugador.dex + self.jugador.dex_b), size=12, color=(209, 195, 137),
                          left=100, top=74 + 15)
        char_w_int = Text(text="Int: " + str(self.jugador.int + self.jugador.int_b), size=12, color=(209, 195, 137),
                          left=20, top=74 + 15 * 2)
        char_w_vit = Text(text="Vit: " + str(self.jugador.vit + self.jugador.vit_b), size=12, color=(209, 195, 137),
                          left=100, top=74 + 15 * 2)
        char_w_agi = Text(text="Agi: " + str(self.jugador.agi + self.jugador.agi_b), size=12, color=(209, 195, 137),
                          left=20, top=74 + 15 * 3)
        char_w_stats = Text(text="Points: " + str(self.jugador.stat_points), size=12, color=(209, 195, 137), left=60,
                            top=74 + 15 * 4)
        char_w_atk = Text(text="Atk: " + str(self.jugador.atack + self.jugador.atack_b), size=12, color=(209, 195, 137),
                          left=20, top=156)
        char_w_def = Text(text="Def: " + str(self.jugador.defense + self.jugador.defense_b), size=12,
                          color=(209, 195, 137), left=100, top=156)
        char_w_lth = Text(text="Lth: " + str(float(self.jugador.crit + self.jugador.crit_b)) + "%", size=12,
                          color=(209, 195, 137), left=20, top=156 + 15)
        char_w_dge = Text(text="Dge: " + str(float(self.jugador.dodge + self.jugador.dodge_b)) + "%", size=12,
                          color=(209, 195, 137), left=100, top=156 + 15)
        char_w_mhp = Text(text="MHP: " + str(self.jugador.maxhp + self.jugador.maxhp_b), size=12, color=(209, 195, 137),
                          left=20, top=156 + 15 * 2)
        char_w_msp = Text(text="MSP: " + str(self.jugador.maxsp + self.jugador.maxsp_b), size=12, color=(209, 195, 137),
                          left=100, top=156 + 15 * 2)
        char_w_SoulE = Text(text="Soul's Essences: " + str(self.jugador.soul_points), size=12, color=(209, 195, 137),
                            left=20, top=209)
        char_w_StoneE = Text(text="Stone's Essences: " + str(self.jugador.stone_points), size=12, color=(209, 195, 137),
                             left=20, top=209 + 15)
        ...
        for y in range(2):
            for x in range(4):
                ...
                self.EquipWindow.appendReceptor(
                    Receptor(Type="Equip", width=24, height=24, left=x * 41 + 23, top=y * 48 + 28))
    ...
    def barra_estado_info(self):
        ...
        HP = Text(text=(str(self.jugador.hp) + "/" + str(self.jugador.maxhp)), size=13, color=(255, 255, 255), left=0,
                  top=5 + y0)
        SP = Text(text=(str(self.jugador.sp) + "/" + str(self.jugador.maxsp)), size=13, color=(255, 255, 255), left=0,
                  top=35 + y0)
        EXP = Text(text=(str(self.jugador.exp) + "/" + str(self.jugador.exp_nivel)), size=13, color=(255, 255, 255),
                   left=0, top=65 + y0)
    ...
    def ObjectsEquip(self, ind):
        ...
        self.EquipWindow.objects[ind] = Object(image=self.jugador.equip[ind].image_inv, left=x, top=y, width=24,
                                               height=24, moveable=False, reference=ind)
        self.EquipWindow.receptors[ind].state = True

    def ObjectsBag(self, ind, ind_i):
        (l, t) = (self.BagWindow.rect.left + 10, self.BagWindow.rect.top + 32)
        (x, y) = ( l + (ind - (ind / 5) * 5) * 27, t + (ind / 5) * 27 )
        self.BagWindow.objects[ind] = Object(image=self.jugador.bag[ind_i].image_inv, left=x, top=y, width=24,
                                             height=24, moveable=True, reference=ind_i)
    ...
    def ItemInfo(self, cursor, (tipe, ind)):
        if tipe == "i":
            ...
            self.ItemInfoWindow = Window(width=0, height=0, left=self.BagWindow.rect.left - 200,
                                         top=self.BagWindow.rect.top + 1, moveable=1)
            self.ShowII_act_b = True
        elif tipe == "c":
            item = self.jugador.equip[self.EquipWindow.objects[ind].reference]
            self.ItemInfoWindow = Window(width=0, height=0, left=self.EquipWindow.rect.right,
                                         top=self.EquipWindow.rect.top + 1, moveable=1)
    ...
    def CharWindowUpdate(self, stats):
        ...
        for ind in stats:
            if ind == 0:
                self.CharWindow.texts[ind] = Text(text="Lv: " + str(self.jugador.nivel), size=12, color=(209, 195, 137),
                                                  left=20 + x, top=53 + y)
            elif ind == 1:
                self.CharWindow.texts[ind] = Text(
                    text="Exp: " + str(round(self.jugador.exp * 100.0 / self.jugador.exp_nivel, 2)) + "%", size=12,
                    color=(209, 195, 137), left=100 + x, top=53 + y)
            elif ind == 2:
                self.CharWindow.texts[ind] = Text(text="Job: " + self.jugador.especializacion, size=12,
                                                  color=(209, 195, 137), left=20 + x, top=53 + 15 + y)
            elif ind == 3:
                self.CharWindow.texts[ind] = Text(text="Str: " + str(self.jugador.str + self.jugador.str_b), size=12,
                                                  color=(209, 195, 137), left=20 + x, top=74 + 15 + y)
            elif ind == 4:
                self.CharWindow.texts[ind] = Text(text="Dex: " + str(self.jugador.dex + self.jugador.dex_b), size=12,
                                                  color=(209, 195, 137), left=100 + x, top=74 + 15 + y)
            elif ind == 5:
                self.CharWindow.texts[ind] = Text(text="Int: " + str(self.jugador.int + self.jugador.int_b), size=12,
                                                  color=(209, 195, 137), left=20 + x, top=74 + 15 * 2 + y)
            elif ind == 6:
                self.CharWindow.texts[ind] = Text(text="Vit: " + str(self.jugador.vit + self.jugador.vit_b), size=12,
                                                  color=(209, 195, 137), left=100 + x, top=74 + 15 * 2 + y)
            elif ind == 7:
                self.CharWindow.texts[ind] = Text(text="Agi: " + str(self.jugador.agi + self.jugador.agi_b), size=12,
                                                  color=(209, 195, 137), left=20 + x, top=74 + 15 * 3 + y)
            elif ind == 8:
                self.CharWindow.texts[ind] = Text(text="Points: " + str(self.jugador.stat_points), size=12,
                                                  color=(209, 195, 137), left=60 + x, top=74 + 15 * 4 + y)
            elif ind == 9:
                self.CharWindow.texts[ind] = Text(text="Atk: " + str(self.jugador.atack + self.jugador.atack_b),
                                                  size=12, color=(209, 195, 137), left=20 + x, top=156 + y)
            elif ind == 10:
                self.CharWindow.texts[ind] = Text(text="Def: " + str(self.jugador.defense + self.jugador.defense_b),
                                                  size=12, color=(209, 195, 137), left=100 + x, top=156 + y)
            elif ind == 11:
                self.CharWindow.texts[ind] = Text(
                    text="Lth: " + str(float(self.jugador.crit + self.jugador.crit_b)) + "%", size=12,
                    color=(209, 195, 137), left=20 + x, top=156 + 15 + y)
            elif ind == 12:
                self.CharWindow.texts[ind] = Text(
                    text="Dge: " + str(float(self.jugador.dodge + self.jugador.dodge_b)) + "%", size=12,
                    color=(209, 195, 137), left=100 + x, top=156 + 15 + y)
            elif ind == 13:
                self.CharWindow.texts[ind] = Text(text="MHP: " + str(self.jugador.maxhp + self.jugador.maxhp_b),
                                                  size=12, color=(209, 195, 137), left=20 + x, top=156 + 15 * 2 + y)
            elif ind == 14:
                self.CharWindow.texts[ind] = Text(text="MSP: " + str(self.jugador.maxsp + self.jugador.maxsp_b),
                                                  size=12, color=(209, 195, 137), left=100 + x, top=156 + 15 * 2 + y)
            elif ind == 15:
                self.CharWindow.texts[ind] = Text(text="Soul's Essences: " + str(self.jugador.soul_points), size=12,
                                                  color=(209, 195, 137), left=20 + x, top=209 + y)
            elif ind == 16:
                self.CharWindow.texts[ind] = Text(text="Stone's Essences: " + str(self.jugador.stone_points), size=12,
                                                  color=(209, 195, 137), left=20 + x, top=209 + 15 + y)
            elif ind == 17:
                self.CharWindow.texts[ind] = Text(text="Control: ", size=12, color=(209, 195, 137), left=20 + x,
                                                  top=209 + 15 * 2 + y)
            elif ind == 18:
                self.CharWindow.texts[ind] = Text(text="Gold: " + str(self.jugador.cash), size=12,
                                                  color=(209, 195, 137), left=20 + x, top=260 + y)
